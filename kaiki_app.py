...
def main():
    ...
    **「結果（売上や点数）」**に対して、**「どの要因（広告や勉強時間）」**がどれくらい効いているのか？
    ...
    if st.button("🚀 分析を開始する", type="primary", use_container_width=True):
        ...
        # === Tab 1: 影響度ランキング (標準化係数) ===
        with tab1:
            st.subheader("結局、何が一番重要なのか？")
            st.markdown("単位（円、人、分など）を無視して、**「純粋な影響力の強さ」**を比較したグラフです。")
            ...
            st.caption("※ 棒が長いほど、結果に対する支配力が強い要因です。右に伸びれば「増やす要因」、左なら「減らす要因」です。")
        ...
        with tab2:
            ...
            for index, row in display_df.iterrows():
                with st.expander(f"📌 **{row['変数名']}** の評価", expanded=True):
                    ...
                    sig_text = "統計的に信頼できます" if is_significant else "偶然の可能性があります (P >= 0.05)"
                    
                    with c1:
                        st.metric("1増えるとどうなる？", f"{row['係数 (傾き)']:.2f}", help="実際の単位での変化量")
                    with c2:
                    ...
                    with c3:
                        ...
                        **【AI解説】**
                        ...
                        影響力の強さは全体の **{abs(row['標準化係数 (影響度)']):.2f}** 倍程度です。
                        """)
        ...
        with tab3:
            ...
            for feature, value in user_inputs.items():
                ...
                sign = "+" if coef >= 0 else ""
                calc_process += f" {sign} ({coef:.2f} × {value})"

            st.markdown("---")
            st.markdown(f"### 🎯 予測される {target_var}")
            st.markdown(f"# **{prediction:,.1f}**")
            
            with st.expander("計算式の詳細を見る"):
                st.code(f"{target_var} = {calc_process}")
        ...
        with tab4:
            ...
            with c_diag2:
                # 統計サマリー
                st.markdown("#### 専門家向け統計レポート")
                st.text(res['model'].summary().tables[1].as_text())
